...
export default async function PembekalAdminPage() {
  ...
  if (supabaseReady()) {
    ...
    claims = await Promise.all(((c || []) as Record<string, unknown>[]).map(async (row) => {
      ...
      const doDoc = row.url_do as string | null;
      if (doc) {
      ...
      if (doDoc) {
        if (/^https?:\/\//i.test(doDoc)) out.do_signed = doDoc;
        else { const { data: d } = await svc.storage.from("supplier-docs").createSignedUrl(doDoc, 3600); out.do_signed = d?.signedUrl || null; }
      }
      return out;
    }));
