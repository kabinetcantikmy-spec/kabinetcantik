...
export default function SupplierRegisterForm({ brand }: { brand: { nama: string; logoUrl: string } }) {
  const [f, setF] = useState({ nama: "", emel: "", jenis: "pembekal", password: "" });
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-paper p-6">
        <div className="w-full max-w-md rounded-2xl border border-ink/10 bg-white p-8 text-center">
          ...
          <h1 className="mt-4 font-display text-xl font-semibold text-ink">Akaun berjaya dicipta!</h1>
          <p className="mt-2 text-ink/60">Log masuk & lengkapkan profil KYB anda (butiran syarikat + dokumen) supaya kami boleh sahkan & luluskan akaun.</p>
    ...
    <div className="flex min-h-screen items-center justify-center bg-paper p-6">
      <form onSubmit={submit} className="w-full max-w-md rounded-2xl border border-ink/10 bg-white p-8 shadow-lg">
        <div className="flex flex-col items-center">
          ...
          <p className="mt-1 text-center text-xs text-ink/50">Langkah 1: cipta akaun. Butiran syarikat & dokumen dilengkapkan selepas log masuk.</p>
        </div>
        <div className="mt-5 grid gap-3">
          ...
          <input type="email" required value={f.emel} onChange={(e) => set("emel", e.target.value)} placeholder="Emel *" className="rounded-lg border border-ink/15 bg-paper px-3 py-2.5 text-sm" />
          <input type="password" required value={f.password} onChange={(e) => set("password", e.target.value)} placeholder="Kata laluan (min 6) *" className="rounded-lg border border-ink/15 bg-paper px-3 py-2.5 text-sm" />
        </div>
        {err && <p className="mt-3 text-sm text-red-600">{err}</p>}
        <button type="submit" disabled={pending} className="btn-brass mt-5 w-full">{pending ? "Menghantar…" : "Cipta Akaun"}</button>
