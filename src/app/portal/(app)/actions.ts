...
import { requireCustomer, createSupabaseServer } from "@/lib/supabaseServer";
import { sendEmail, emailShell } from "@/lib/email";
import { revalidatePath } from "next/cache";

type Res = { ok: boolean; error?: string };

async function ownsProject(sb: ReturnType<typeof createSupabaseServer>, projectId: string, customerId: string) {
...
async function notifyStaff(sb: ReturnType<typeof createSupabaseServer>, admin: ReturnType<typeof createServiceClient>, projectId: string, nama: string, mesej: string) {
  const { data: proj } = await sb.from("projects").select("lead_id, tajuk, org_id").eq("id", projectId).single();
  if (proj?.lead_id) {
    await admin.from("lead_activity").insert({ lead_id: proj.lead_id, oleh: nama, jenis: "note", mesej, org_id: proj.org_id });
  }
...
export async function approveDesign(designId: string): Promise<Res> {
  ...
  await notifyStaff(sb, admin, d.project_id, ctx.nama, `${ctx.nama} telah MELULUSKAN reka bentuk.`);
...
export async function requestRevision(designId: string, komen: string): Promise<Res> {
  ...
  if (error) return { ok: false, error: error.message };
  await notifyStaff(sb, admin, d.project_id, ctx.nama, `${ctx.nama} MINTA UBAH reka bentuk: "${komen.trim()}"`);
  revalidatePath("/portal/design");
  return { ok: true };
}
